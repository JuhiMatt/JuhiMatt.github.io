...
aisles <- instacart %>% 
  ...
  plot_ly(x = ~department, y = ~n_aisles, color = ~department, type = "bar", colors = "plasma") %>% #plot
  layout(
...
instacart %>%
  ...
  plot_ly(x = ~order_dow, y = ~n_items,  type = 'box', boxmean = TRUE, color = ~factor(order_dow), colors = "viridis") %>% 
  layout(
    title = "Median Number of Items Per Order by Day of Week",
    xaxis = list(title = "Day",
      tickvals = 0:6,
      ticktext = c("Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday")),
